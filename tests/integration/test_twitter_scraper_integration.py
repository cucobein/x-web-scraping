...
class TestTwitterScraperIntegration:
    ...
    @pytest.mark.asyncio
    async def test_extract_tweet_from_nasa_profile(self, scraper, browser_manager, nasa_html_path):
    ...
    @pytest.mark.asyncio
    async def test_extract_tweet_from_elonmusk_profile(self, scraper, browser_manager, elonmusk_html_path):
        """Test extracting tweet from real Elon Musk profile HTML"""
        # Verify fixture exists
        assert elonmusk_html_path.exists(), f"Elon Musk fixture not found: {elonmusk_html_path}"
        ...
        try:
            # Use the new method for HTML content
            tweet = await scraper.get_latest_tweet_from_html(page, "elonmusk", html_content)
    ...
    @pytest.mark.asyncio
    async def test_tweet_unique_id_generation(self, scraper, browser_manager, nasa_html_path):
    ...
    @pytest.mark.asyncio
    async def test_handle_profile_with_no_posts(self, scraper, browser_manager, no_posts_html_path):
        """Test handling of profile with no posts - using real fixture"""
        # Verify fixture exists
        assert no_posts_html_path.exists(), f"No posts fixture not found: {no_posts_html_path}"
        ...
        try:
            # Use the new method for HTML content
            tweet = await scraper.get_latest_tweet_from_html(page, "no_posts_user", html_content)
    ...
    @pytest.mark.asyncio
    async def test_handle_non_existing_user(self, scraper, browser_manager, non_existing_user_html_path):
        """Test handling of non-existing user - using real fixture"""
        # Verify fixture exists
        assert non_existing_user_html_path.exists(), f"Non-existing user fixture not found: {non_existing_user_html_path}"
        ...
        try:
            # Use the new method for HTML content
            tweet = await scraper.get_latest_tweet_from_html(page, "non_existing_user", html_content)
