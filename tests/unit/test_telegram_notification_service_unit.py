...
class TestTelegramNotificationService:
    ...
    def test_service_initialization(self, telegram_service):
        ...
        assert telegram_service.endpoint == "https://api-com-notifications.mobzilla.com/api/Telegram/SendMessage"
        assert telegram_service.api_key == "test-api-key"
        assert telegram_service.http_client is not None
    
    @pytest.mark.asyncio
    async def test_send_tweet_notification_success(self, telegram_service, sample_tweet):
        """Test successful tweet notification"""
        # Mock successful HTTP response
        mock_response = (200, {"success": True, "message": "Notification sent"})
        
        with patch.object(telegram_service.http_client, 'post_form_data', new=AsyncMock(return_value=mock_response)):
    ...
    @pytest.mark.asyncio
    async def test_send_tweet_notification_retry_success(self, telegram_service, sample_tweet):
        ...
        mock_post.side_effect = [
            ...
            (200, {"success": True, "message": "Notification sent"})  # Third attempt succeeds
        ]
        
        with patch.object(telegram_service.http_client, 'post_form_data', new=mock_post):
            result = await telegram_service.send_tweet_notification(sample_tweet)
    ...
    @pytest.mark.asyncio
    async def test_send_tweet_notification_retry_exhausted(self, telegram_service, sample_tweet):
    ...
    async def test_send_telegram_request_success(self, telegram_service):
        ...
        with patch.object(telegram_service.http_client, 'post_form_data', new=AsyncMock(return_value=mock_response)):
            status_code, response_data = await telegram_service._send_telegram_request(request, headers)
